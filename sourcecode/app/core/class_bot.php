...
class bot {
	...
	public function update_spotify_widget( $data ){
		...
		foreach( $playlist_data["tracks"] as $playlist_track_simple ){
			...
			if ( !is_array( $album_verified_args = $this->loader->album->verify_args( $album_args ) ) ){
				$this->__log( "There is something wrong with this album's info or MeMusic does not support it. Skipping it", true );
			...
			if ( !is_array( $album_db_data = $this->loader->album->create( $album_verified_args ) ) ){
				$this->__log( "MeMusic failed to create this album on database, Skipping it", true );
				$__report["ignored"]++;
				continue;
			}

			if ( empty( $album_db_data["ID"] )  ){
				$this->__log( "MeMusic failed to create this album on database ( 2 ), Skipping it", true );
				$__report["ignored"]++;
				continue;
			}
			...
			if ( !is_array( $track_verified_args = $this->loader->track->verify_args( $track_args ) ) ){
				$this->__log( "There is something wrong with this track's info or MeMusic does not support it. Skipping it", true );
			...
			if ( !is_array( $track_db_data = $this->loader->track->create( $track_verified_args ) ) ){
				$this->__log( "MeMusic failed to create this track on database, Skipping it", true );
				$__report["ignored"]++;
				continue;
			}
	...
	public function localize_video( $track_data, $youtube_id ){
		...
		if ( substr( $youtube_file_path, -4 ) == ".mp3" ){
			...
			exec( "{$ffmpeg_location} -i \"{$youtube_file_path}\" -acodec mp3 -vn \"{$youtube_mp3_path}\" " );
			$youtube_mp3_path = realpath( $youtube_mp3_path );
			unlink( $youtube_file_path );

		} if ( empty( $youtube_mp3_path ) ) return [ false, "no_mp3_found" ];
