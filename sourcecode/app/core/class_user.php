...
class user {
	...
	protected function select_clean( $data, $external = [] ){
		...
		if ( in_array( "group_data", $external ) ){
			...
			if ( !empty( $data["group_data"]["verified"] ) )
	...
	public function get_acts( $args ){
      ...
      if ( empty( $user_actions[ $action["type"] ] ) ) continue;
			...
			elseif ( $action_type_data["hook_type"] == "playlist" ){
				$action["data"]["hook"] = $action["data"]["playlist"] = $this->loader->playlist->select( [ "ID" => $action["hook"], "_eg" => [ "owner" ] ] );
				$action["data"]["hook"]["simplified"] = [
			...
			elseif ( $action_type_data["hook_type"] == "ads" ){
				$action["data"]["hook"] = $action["data"]["ads"] = $this->loader->ads->select( [ "ID" => $action["hook"] ] );
				$action["data"]["hook"]["simplified"] = [
	...
	public function notify( $args ){
		...
		if ( $via == "email" || $via == "all" ){
			...
			$send = $this->notify_email( $email, $subject, $content );
	...
	protected function notify_email( $to, $_subject, $content, $from=null ){
		...
		if ( $email_s_type == "mail" || empty( $email_s_host ) || empty( $email_s_port ) ){
			...
			try {
				...
				$mail->SMTPSecure = $email_s_encrypt == "tls" ? PHPMailer::ENCRYPTION_STARTTLS : PHPMailer::ENCRYPTION_STARTTLS;
