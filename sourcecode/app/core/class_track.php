...
class track {
	...
	public function select_data( $track_id ){
		...
		if ( $data["comment"] ){
			$data["comment"] = str_replace( PHP_EOL, "<BR>", $this->loader->general->json_decode( $data["comment"] ) );
			$data["comment"] = $this->loader->secure->escape( $data["comment"] );
		}

		if ( $data["lyrics"] ){
			$data["lyrics"] = str_replace( PHP_EOL, "<BR>", $this->loader->general->json_decode( $data["lyrics"] ) );
			$data["lyrics"] = $this->loader->secure->escape( $data["lyrics"] );
		}
